...
Panel {
  ...
  function toggleDisplay(name, enabled) {
    ...
    if (enabled) {
      actionProc.command = ["hyprctl", "keyword", "monitor", name + ",disable"]
    } else {
      // Re-enabling with the bare ",preferred,auto,auto" keyword forces native
      // resolution, which for the Studio Display exceeds this GPU's non-DSC
      // DP link bandwidth and flickers. Reload monitors.lua right after so any
      // per-output override (forced mode/scale) is reasserted immediately.
      actionProc.command = ["bash", "-c", "hyprctl keyword monitor " + name + ",preferred,auto,auto && hyprctl reload"]
    }
